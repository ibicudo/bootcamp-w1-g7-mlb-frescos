...
import static com.mercadolibre.bootcamp_w1_g7_mlb_frescos.util.ScopeUtils.SCOPE_VALUE;
import static com.mercadolibre.bootcamp_w1_g7_mlb_frescos.util.ScopeUtils.isLocalScope;
...
public class SpringDocConfig {
	...
	public OpenAPI customOpenAPI(@Value("${app.title}") String appName, @Value("${app.description}") String description, @Value("${app.version}") String version) {
		OpenAPI api = new OpenAPI().info(new Info().title(appName)
				...
				.contact(new Contact().name("bootcamp-w1-g7-mlb-frescos")
						.email("lucas.mantovani@mercadolivre.com")));
